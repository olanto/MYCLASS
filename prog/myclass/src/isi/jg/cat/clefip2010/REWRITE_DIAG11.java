 ...
 */
public class REWRITE_DIAG11 {

    private static final int MaxInCache = 1000000;
    ...
    public static void main(String[] args) {
        ...
        rewritemflf("C:/TEMPOMFLF/rewrite_frende_16_30.mflf", 24, 20);
        t1.stop();
    }
    ...
    private static final void rewritemflf(String f, int minocc, int maxocc) {
        ...
        try {
            ...
            for (int i = 0; i < id.lastRecordedDoc; i++) {
                if (i % 1000 == 0) {
                    msg("doc rewrite:" + i + ", countrealnext:" + countrealnext + ", countnonext:" + countnonext + ", in cache:" + nextocc.size());
                ...
                if (seq.length > 0) {
                    ...
                    if (nbocc >= minocc && (long) nbocc * 1000l / (long) id.lastRecordedDoc < maxocc) { // terme atteind le seuil
                    ...
                    for (int j = 1; j < seq.length; j++) {
                        ...
                        if (nbocc >= minocc && (long) nbocc * 1000l / (long) id.lastRecordedDoc < maxocc) { // terme atteind le seuil
                            ...
                            if (nbocc >= minocc && (long) nbocc * 1000l / (long) id.lastRecordedDoc < maxocc) { // terme-1 atteind le seuil
