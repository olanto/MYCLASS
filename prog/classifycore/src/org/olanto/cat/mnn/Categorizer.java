...
public class Categorizer {
    ...
    /**
     * Initialise un catégoriseur associé avec un fichier contenant son réseau
     * de neurones
     *
     * @param _fileName Fichier du réseau de neurones
     * @param _fullload true -> le réseau de neurone est chargé en mémoire false
     * -> les neurones sont chargées une à une
     * @param _MAXCLASS Nombre de classe maximum (ce nombre est déterminé à la
     * construction du réseau de neurones)
     * @param _MAXFEATURE taille du cache des neurones (2000)
     *
     * @param _STARTLEVEL niveau de fréquence pour initialiser le nettoyage du
     * cache (16)
    ...
    public Categorizer(
        ...
        if (fullload) {
            fullLoadCompress();  // charge toutes les features compressée
        }
    }
    ...
    private void resetFeatureALLForce() {
    ...
    private void load() {
        try {
            ...
            if (wordAtIdx == null) {
                System.out.println("wordAtIdx is null -> DirectMode");
                directMode = true;
            }
            ...
            if (worduse == null) {
                System.out.println("worduse is null -> DirectMode");
                directMode = true;
            }
            if (directMode) {
                System.out.println("DirectMode set worduse to null");
                worduse = null;
            }
       BytesAndFiles.compactMemory("end worduse load:");
    ...
    private synchronized final float[] computeWinnowPosCompact(int[] docbag, int NNidx) {
        if (directMode){
            return computeWinnowPosCompactDirect(docbag,NNidx);
        }
        else{
            return computeWinnowPosCompactInDirect(docbag,NNidx);
        }
                
    }
    
    
        private synchronized final float[] computeWinnowPosCompactDirect(int[] docbag, int NNidx) {
...
//        for (int i=0;i<cumul.length;i++)System.out.println(cumul[i]);
    ...
    private synchronized final float[] computeWinnowPosCompactInDirect(int[] docbag, int NNidx) {
//        System.out.println("NNidx: "+NNidx);
//        System.out.println("maxgroup[NNidx]: "+maxgroup[NNidx]);
        resetFeature();
...
//       }
    ...
    /**
     * Demande la classification d'un document en suivant une stratégie de
     * parcours de l'arbre de décision. (pour les tests/recherches uniquement)
     *
