...
//
    ...
    public static void testWinnow4(boolean detailclass, boolean detaildocument, int n) {
        ...
        OutputStreamWriter resultdoc = null;
...
//                    detaildoc.write("docname;main;choice1;level1;choice2;level2;choice3;level3;docbagsize\n");
                ...
                if (detaildocument) { // sauve dans le fichier
                    if (collectResult != null) {
                        try {
                            ...
                            resultdoc.write(glue.getFileNameForDocument(d));
                            for (int k = 0; k < n; k++) {
                                if (top[k] != NOT_FOUND) {
                                    detaildoc.write(";" + ActiveGroup.getgroupName(top[k]) + ";" + ((int) cumulShared[top[k]]));
                                    resultdoc.write("\t" + ActiveGroup.getgroupName(top[k]));
                                }
    ...
    public static void exportDocSVM(OutputStreamWriter exportdoc, int[] docbag, int gk) {
        try {
            ...
            Logger.getLogger(NNOneN.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public static void testWinnowPR(boolean detailclass, int n) {
        ...
        for (int id = beg; id < end; id++) {
            ...
            if (testDocOK2(d)) {
                totdoc++;
                int[] g = ActiveGroup.getGroup(d);
                for (int i = 0; i < g.length; i++) {  // calcul toutes les classes ? trouver
                ...
                for (int k = 0; k < n; k++) {
                    for (int i = 0; i < g.length; i++) {
                        if (g[i] == top[k]) {
                            ...
                            inclass[g[i]][k]++;   // ajoute au score du groupe du guess
                        }
                    }
                }
            }
        ...
        for (int i = 0; i < maxgroup; i++) {  // calcul tous les symbols ? trouver
        ...
        if (detailclass) {
            if (collectResult != null) {
                String filename = collectResult.pathfileSave + detailFolderName + collectResult.experimentName + "-Detail-ClassPR.txt";
                try {
                    OutputStreamWriter detaildoc = new OutputStreamWriter(new FileOutputStream(filename), "UTF-8");
                    System.out.println("detail in: " + filename);
                    detaildoc.write("group,tottest,in1,in2,in3,...\n");
