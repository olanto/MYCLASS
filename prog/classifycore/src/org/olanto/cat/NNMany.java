...
class NNMany {
    ...
    NNMany(String _NNName, int _mode, String _fileName,
        ...
        if (mode == MODE_LEARN) {
            ...
            maxused = NNOneN.maxused;
            wordAtIdx = NNOneN.wordAtIdx;
            wordFreq = NNOneN.wordFreq;
            sdf = NNOneN.sdf;
            alfaPn = NNOneN.alfaPn;
            alfaNn = NNOneN.alfaNn;
            try {
    ...
    void RecordNN() {
        String NetName = NNOneN.categorylevel + "." + NNOneN.prefix;
        ...
        if (n == null) { // a new one
            if (lastNNT < MAXNN) {
                ...
                // copy ref from group
                groupinv[lastNNT] = NNOneN.ActiveGroup.groupinv;
                // copy ref from NNOne
                worduse[lastNNT] = NNOneN.worduse;
                // nnc[lastNNT]=NNOne.nnc;
                maxgroup[lastNNT] = NNOneN.maxgroup;
                if (lastfreemnn + NNOneN.maxgroup <= MAXCLASS) {
                    try {
                        // transpose before saving
                        byte[] btr = new byte[maxused];
                        for (int j = 0; j < NNOneN.maxgroup; j++) { // for each group
                            for (int i = 0; i < maxused; i++) { // for each feature
                                btr[i] = NNOneN.nnc[i][j];
                            }
