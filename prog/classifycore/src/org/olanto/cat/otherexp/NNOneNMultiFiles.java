 ...
 *
 */
public class NNOneNMultiFiles {

    private static ExperimentMultiFiles collectResult;
     ...
     */
    public static ExperimentMultiFiles getCollectResult() {
     ...
     */
    public static void setCollectResult(ExperimentMultiFiles _collectResult) {
     ...
     * @return the glue
     */
    public static IdxStructure getGlue() {
        return glue;
    }
     ...
     */
    public static final int CAT_MAINGROUP = 8;
    ...
    static float[] wordWeigth;   // idf weigth
...
//    static int[] docbag;          // active docbag (for train, correct, and evaluation)
    ...
    public static void init(String _s, NNBottomGroup _BootGroup, IdxStructure _glue, boolean _normalised, int _method) {
        ...
        lastdoc = getGlue().lastRecordedDoc;
        lastword = getGlue().lastRecordedWord;
        normalisedFeature = _normalised;
        method = _method;
        if (INMEMORY) {
            alldocbag = new int[lastdoc][];
            for (int d = 1; d < lastdoc; d++) {
                alldocbag[d] = getGlue().getBagOfDoc(d);
            }
        }
    ...
    public static void initNNBottomGroup(NNBottomGroup _BootGroup) {
               BootGroup = _BootGroup;
        avgLength();
       ...
       //      for(int id=maxtrain;id<lastdoc;id++){ // use test vocabulary
        ...
        for (int j = 0; j < lastword; j++) {// compute freq
            ...
            if (wordAtIdx[j] != NOT_FOUND) {
                wordFreq[wordAtIdx[j]] = getGlue().getOccOfW(j); // document appearance
    ...
    public static void TrainWinnow(int _categorylevel, String _prefix, int repeatK, float _qlevel, float _add, int _minocc, int _maxocc,
        ...
        BytesAndFiles.usedMemory("start mem");
        prefix = _prefix;
        categorylevel = _categorylevel;
        System.out.println(categorylevel + "." + prefix);
        RandomizeDoc.init(getGlue(), BootGroup, prefix);
        ...
        ActiveGroup = new NNLocalGroup(BootGroup, getGlue(), categorylevel, verbosetrain);

        BytesAndFiles.usedMemory("after localgroup");
...
//
        ...
        start = System.currentTimeMillis() - start + 1;
        ...
        for (int id = beg; id < end; id++) {
            ...
            if (testDocOK2(d)) {
                ...
                if (detail) {
                    ...
                    System.out.println(getGlue().getFileNameForDocument(d) + " " + ActiveGroup.getgroupName(g) +
    ...
    public static String[] getTopForString(String request, int nfirst) {
        ...
        DoParse a = new DoParse(request + ".", getGlue().dontIndexThis);
        int[] db = a.getDocBag(getGlue()); // get the docBag of the request
        //showVector(db);
    ...
    public static void testWinnow4(boolean detailclass, boolean detaildocument, int n) {
        ...
        if (detaildocument) { //ouvre le fichier
            if (collectResult != null) {
                String filename = collectResult.pathfileSave + detailFolderName + collectResult.experimentName + "-MainDetail-Doc.csv";
                try {
        ...
        for (int id = beg; id < end; id++) {
            ...
            if (testDocOK2(d)) {
                ...
                if (detaildocument) { // sauve dans le fichier
                    if (collectResult != null) {
                        try {
                            detaildoc.write(getGlue().getFileNameForDocument(d) + ";" + ActiveGroup.getgroupName(g));
                            for (int k = 0; k < n; k++) {
                                if (top[k] != NOT_FOUND) {
                            ...
                            int[] docbag=getGlue().getBagOfDoc(d);
    ...
    public static void exportTrainSVM(String filename) {

        // init
        if (wordWeigth==null)initWordWeigth();
        OutputStreamWriter exportdoc = null;
        try {
            exportdoc = new OutputStreamWriter(new FileOutputStream(filename));
        } catch (Exception e) {
            System.err.print("error in save train SVM ...");
        }

        int beg = 0;
        int end = lasttesttraindoc;
        int totdoc = 0;
        try {
            for (int id = beg; id < end; id++) {
                int d = RandomizeDoc.rand[id];
                if (testDocOK2(d)) {
                    ...
                    for (int k = 0; k < g.length; k++) {
                        exportDocSVM(exportdoc, docbag, g[k]);
                    }
                }
            }
            exportdoc.close();
    ...
    public static void initWordWeigth() {
        ...
        for (int j = 0; j < lastword; j++) {// compute freq
            if (wordAtIdx[j] != NOT_FOUND) {
                wordWeigth[wordAtIdx[j]] = (float) Math.log(1 + (double) lastdoc / (double) wordFreq[wordAtIdx[j]]);
            }
        }
    }
    ...
    public static void exportDocSVM(OutputStreamWriter exportdoc, int[] docbag, int gk) {
        try {
            ...
            for (int i = 0; i < docbag.length; i++) {
                int iiii = wordAtIdx[docbag[i] / DocBag.MAXOCCINDOC]; // with indirection
                if (iiii != NOT_FOUND && worduse[iiii] >= 0x4) {
                    // if not discard
                   // norm += (float) Math.sqrt((double) docbag[i] % DocBag.MAXOCCINDOC)*wordWeigth[iiii];
                    norm += docbag[i] % DocBag.MAXOCCINDOC;
                }
            }
            ...
            for (int i = 0; i < docbag.length; i++) {
                ...
                if (iiii != NOT_FOUND && worduse[iiii] >= 0x4) {
                    // if not discard
                    //exportdoc.write(" " + (wordAtIdx[docbag[i] / DocBag.MAXOCCINDOC] + 1) + ":" + (float) Math.sqrt((double) docbag[i] % DocBag.MAXOCCINDOC)*wordWeigth[iiii] / norm);  //idf*sqrt(tf)/norm
                    exportdoc.write(" " + (wordAtIdx[docbag[i] / DocBag.MAXOCCINDOC] + 1) + ":" + Math.sqrt((double) docbag[i] % DocBag.MAXOCCINDOC)*wordWeigth[iiii]);  // term freq
                }
            }
            exportdoc.write("\n");
        } catch (IOException ex) {
            Logger.getLogger(NNOneNMultiFiles.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public static void exportTestSVM(String filename) {  // export main group
        // init
         if (wordWeigth==null)initWordWeigth();
        OutputStreamWriter exportdoc = null;
        try {
        ...
        try {
            for (int id = beg; id < end; id++) {
                ...
                if (testDocOK2(d)) {
                    ...
                    int[] g = ActiveGroup.getGroup(d);
                    int[] docbag = alldocbag[d];
                    for (int k = 0; k < 1; k++) {  // limité au main group
                        exportDocSVM(exportdoc, docbag, g[k]);
                    }
                }
            }
    ...
    public static void testWinnowTree(boolean testtrain, Categorizer MM, int mode, int classLevel) {
        RandomizeDoc.init(getGlue(), BootGroup, ""); // reset all preceding computation, get all doc

        lasttesttraindoc = RandomizeDoc.lasttraindoc;  // 100 % je t aime

        maxtrain = RandomizeDoc.lasttraindoc;
        lasttestdoc = RandomizeDoc.lasttestdoc;
        ActiveGroup = new NNLocalGroup(BootGroup, getGlue(), classLevel, false);
        ...
        for (int id = beg; id < end; id++) {
            int d = RandomizeDoc.rand[id];
            int g = ActiveGroup.getMainGroup(d);
            String sg = ((String) ActiveGroup.groupinv.get(new Integer(g)));
            if (testDocOK2(d)) {
                int[] docbag = alldocbag[d];

                String stop = "";
                if (NNBuildTree.MODE_134 == mode) {
                    stop = MM.computeWinnowTree_134(docbag);
                }
                if (NNBuildTree.MODE_104 == mode) {
                    stop = MM.computeWinnowTree_104(docbag);
                }
                if (NNBuildTree.MODE_034 == mode) {
                    stop = MM.computeWinnowTree_034(docbag);
                }
                if (NNBuildTree.MODE_P34 == mode) {
                    stop = MM.computeWinnowTree_P34(docbag, sg.substring(0, 1));
                }
                if (NNBuildTree.MODE_P04 == mode) {
                    stop = MM.computeWinnowTree_P04(docbag, sg.substring(0, 1));
                }
                if (NNBuildTree.MODE_1 == mode) {
                    stop = MM.computeWinnowTree_1(docbag);
                }
                int top = -1;
                if (stop != null) {
                    top = ActiveGroup.getgroup(stop);
                }
                totingroup++;
                if (top == -1) {
                    totnoclassgroup++;
    ...
    public static void generateIndex(boolean verbosemode, int nKW) {
        ...
        for (int id = beg; id < end; id++) {
            ...
            int g = ActiveGroup.getMainGroup(d);
            if (testDocOK2(d)) {
                System.out.println("#####" + getGlue().getFileNameForDocument(d) + "#####" +
                        "\n<MR:class id=\"" + ActiveGroup.getgroupName(g) + "\">");
                ww = computeWinnowWeight(d, g);
                if (verbosemode) {
                    ww.displayXML(getGlue(), nKW);
                }
            }
        }
    }
    ...
    public static void explainGroup(int nKW) {
        ...
        for (int i = 0; i < maxgroup; i++) {
            ...
            ww = computeWinnowWeight(i);
            ww.displayTXTDetail(getGlue(), nKW);
            System.out.println();
