 ...
 *
 */
public class NNOneN_other_experiment {

    private static Experimentshort collectResult;
     ...
     */
    public static Experimentshort getCollectResult() {
     ...
     */
    public static void setCollectResult(Experimentshort _collectResult) {
    ...
    public final static int formula = 3;
    ...
    static float[] wordWeigth;       // map for compacting words freq
...
//    static int[] docbag;          // active docbag (for train, correct, and evaluation)
    ...
    static final void computeWinnowPosCompactOnVector(float[] cumul, int[] docbag) {
        ...
        for (int i = 0; i < docbag.length; i++) {
            int iiii = wordAtIdx[docbag[i] / DocBag.MAXOCCINDOC];  // with indirection
            if (iiii != NOT_FOUND && worduse[iiii] >= 0x4) {// if not discard
                float wsdf = sdf[docbag[i] % DocBag.MAXOCCINDOC]*wordWeigth[iiii]; // eval feature weight
                // if  (DISCARDPROCESS) worduse[iiii]=0x8;
                for (int j = 0; j < maxgroup; j++) {
                    if (nnc[iiii][j] < 0)// negative
                    {
    ...
    static final void normalisedFeature(float[] cumul, int d) {
        if (normalisedFeature) {
            ...
            for (int i = 0; i < docbag.length; i++) {  // compute normalisation for this doc
                int iiii = wordAtIdx[docbag[i] / DocBag.MAXOCCINDOC];  // with indirection
                if (iiii != NOT_FOUND && worduse[iiii] >= 0x4) {// if not discard
                    normalised += sdf[docbag[i] % DocBag.MAXOCCINDOC]*wordWeigth[iiii];
                }
            }
        ...
        for (int j = 0; j < lastword; j++) {// compute freq
            ...
            if (wordAtIdx[j] != NOT_FOUND) {
                ...
                if (formula == 2) { //  ln(N/f(t)-1)
                    wordWeigth[wordAtIdx[j]] = (float) Math.log((double) lastdoc / (double) wordFreq[wordAtIdx[j]] - 1);
                }
                if (formula == 3) { //  ln(N/f(t)-1)
                  wordWeigth[wordAtIdx[j]] = 1 ;
//                    wordWeigth[wordAtIdx[j]] = (float)(1/ Math.sqrt((float) wordFreq[wordAtIdx[j]]));
//                    wordWeigth[wordAtIdx[j]] = (float)Math.sqrt((float) wordFreq[wordAtIdx[j]]);
//                    wordWeigth[wordAtIdx[j]] = (float)Math.sqrt((float) wordFreq[wordAtIdx[j]]);
//                    wordWeigth[wordAtIdx[j]] =(float) Math.sqrt((float) Math.log((double) lastdoc / (double) wordFreq[wordAtIdx[j]] - 1));
        ...
        for (int i = 0; i < MAXBYTE; i++) {  // init power of alfa
            alfaPn[i + 1] = alfaPn[i] * newadd;
            alfaNn[i + 1] = alfaNn[i] / newadd;  // min is not used symetric promotion and demotion
            newadd/=accelerator;
        }
        t1.stop();
