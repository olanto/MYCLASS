 ...
 *
 */
public class NNOneN_OverSampling {

    private static ExperimentOver collectResult;
     ...
     */
    public static ExperimentOver getCollectResult() {
     ...
     */
    public static void setCollectResult(ExperimentOver _collectResult) {
        collectResult = _collectResult;
    }

    /**
     * @param aOverSampling the overSampling to set
     */
    public static void setOverSampling(boolean aOverSampling) {
        overSampling = aOverSampling;
    }

    /**
     * @param aMaxOver the maxOver to set
     */
    public static void setMaxOver(int aMaxOver) {
        maxOver = aMaxOver;
    }
...
//    static int[] docbag;          // active docbag (for train, correct, and evaluation)
    ...
    static int overSampling(NNLocalGroup ActiveGroup, int maxgroup,
            int lasttesttraindoc, int lasttestdoc,
            int minsize) {
        boolean verbose = false;
        Vector[] dg = new Vector[maxgroup];
        for (int i = 0; i < maxgroup; i++) {
            dg[i] = new Vector();
        }
        for (int i = 0; i < lasttesttraindoc; i++) {
            int g = ActiveGroup.getMainGroup(RandomizeDoc.rand[i]);
            if (g != -1) {
                //System.out.println(g);
                dg[g].add(RandomizeDoc.rand[i]);
            }
        }
        if (verbose) {
            for (int i = 0; i < maxgroup; i++) {
                System.out.println("group:" + i + " / " + dg[i].size());
            }
        }
        for (int g = 0; g < maxgroup; g++) {
            if (dg[g].size() < minsize) {
                int addsample = minsize - dg[g].size();
                int last = dg[g].size();
                if (last != 0) {
                    for (int i = 0; i < addsample; i++) {
                        int addi = i % last;
                        dg[g].add(dg[g].get((addi)));
                    }
                }
            }
        }
        ...
        int[] newrand = new int[countall + lasttestdoc - lasttesttraindoc];
        // copy
        int current = 0;
        for (int g = 0; g < maxgroup; g++) {
            for (int i = 0; i < dg[g].size(); i++) {
                newrand[current] = (Integer) dg[g].get(i);
                current++;
            }
        }
        for (int i = lasttesttraindoc; i < lasttestdoc; i++) {
            newrand[current] = RandomizeDoc.rand[i];
            current++;
        }
        int shuffle = countall; // randomize traindoc
        ...
        RandomizeDoc.rand = newrand;
        RandomizeDoc.lasttestdoc = newrand.length;
        RandomizeDoc.lasttraindoc = newrand.length;
    ...
    public static void TrainWinnow(int _categorylevel, String _prefix, int repeatK, float _qlevel, float _add, int _minocc, int _maxocc,
        ...
        if (overSampling) {
            maxtrain = overSampling(ActiveGroup, maxgroup, lasttesttraindoc, lasttestdoc, maxOver);
            lasttesttraindoc = maxtrain;
            lasttestdoc = RandomizeDoc.lasttraindoc;
            System.out.println("after oversampling Train 0.." + lasttesttraindoc + " Test .." + lasttestdoc);
            if (verbosetrain) {
                showRepartitionTX();
            }
        }
...
//
    ...
    public static void testWinnow4(boolean detailclass, boolean detaildocument, int n) {
        ...
        if (detaildocument) { //ouvre le fichier
            if (collectResult != null) {
                String filename = collectResult.pathfileSave + detailFolderName + collectResult.experimentName + "-MainDetail-Doc.txt";
                try {
        ...
        for (int i = 0; i < maxgroup; i++) {
            for (int k = 0; k < n; k++) {
                if (totclass[i] != 0) {
                    avg[k] += (float) inclass[i][k] / (float) totclass[i] / (float) maxgroup;
                }
            }
        }
        float totavg = 0f;
        ...
        System.out.print("Mainclass avg," + totavg);
        for (int k = 0; k < n; k++) {
        ...
        if (collectResult != null) {
            ...
            collectResult.AvgMainTotal = 100*totavg;
            if (n >= 3) {
                collectResult.AvgMain1 = 100*avg[0];
                collectResult.AvgMain2 = 100*avg[1];
                collectResult.AvgMain3 = 100*avg[2];
            }
        }
    ...
    public static void testWinnow4Multi(boolean detailclass, int n) {
        ...
        System.out.print("Multiclass avg," + totavg);
        for (int k = 0; k < n; k++) {
            System.out.print("," + avg[k]);
        }
        System.out.println();
        if (collectResult != null) {
            ...
            collectResult.AvgMultiTotal = 100*totavg;
            if (n >= 3) {
                collectResult.AvgMulti1 = 100*avg[0];
                collectResult.AvgMulti2 = 100*avg[1];
                collectResult.AvgMulti3 = 100*avg[2];
            }
        }
