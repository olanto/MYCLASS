 ...
 *
 */
public class ExperimentBal extends Object {
   ...
   //belong to this
    ...
    public ExperimentBal(String experimentName,
            String pathfileSave,
            int nbproc,
            boolean inmemory,
            int categorylevel,
    ...
    public void doIt() {
        NNOneNBal.setCollectResult(this);
        NNOneNBal.setNB_PROC(nbproc);
        NNOneNBal.setINMEMORY(inmemory);
        NNOneNBal.TrainWinnow(categorylevel, prefix, repeatK, qlevel, add, minocc, maxocc, deltamin, deltamax, verbosetrain, testtrain, trainpart);

        if (maintest) {
            NNOneNBal.testWinnow4(maintestGroupdetail, maintestDocumentdetail, Nfirst);
        }
        if (multitest) {
            NNOneNBal.testWinnow4Multi(multitestGroupdetail, Nfirst);
        }

    }

    public static List<ExperimentBal> loadSetOfExperiment(String fname) {
        expFileName = fname;
        Vector<ExperimentBal> res = new Vector<ExperimentBal>();
        try {
